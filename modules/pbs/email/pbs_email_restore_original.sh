...
clear
echo -e "${MENU}=== PBS: Restauração da Configuração Original do E-mail (Postfix) ===${NORMAL}"
...
echo -e "${RED_TEXT}a partir dos backups criados por este conjunto de scripts (main.cf.BCK_pbs_scripts e aliases.BCK_pbs_scripts).${NORMAL}"
...
if [ -f /etc/postfix/main.cf.BCK_pbs_scripts ]; then
    echo "Restaurando /etc/postfix/main.cf..."
    cp -f /etc/postfix/main.cf.BCK_pbs_scripts /etc/postfix/main.cf
    echo "/etc/postfix/main.cf restaurado."
    restored_something=1
else
    echo -e "${YELLOW_TEXT}Backup /etc/postfix/main.cf.BCK_pbs_scripts não encontrado. Nenhuma restauração para main.cf.${NORMAL}"
fi

if [ -f /etc/aliases.BCK_pbs_scripts ]; then
    echo "Restaurando /etc/aliases..."
    cp -f /etc/aliases.BCK_pbs_scripts /etc/aliases
    newaliases # Apply alias changes
    echo "/etc/aliases restaurado e 'newaliases' executado."
    restored_something=1
else
    echo -e "${YELLOW_TEXT}Backup /etc/aliases.BCK_pbs_scripts não encontrado. Nenhuma restauração para aliases.${NORMAL}"
fi
...
echo ""
echo -e "${MENU}=== Restauração PBS concluída. ===${NORMAL}"
read -n 1 -s -r -p "Pressione uma tecla para continuar..."
