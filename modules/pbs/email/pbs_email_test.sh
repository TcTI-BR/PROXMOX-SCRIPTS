...
clear
echo -e "${MENU}=== PBS: Teste de Envio de E-mail ===${NORMAL}"
...
SUBJECT_EMAIL="PBS Teste de e-mail - $hostname - $date" # Subject changed for PBS
BODY_EMAIL="Este e-mail confirma que a configuração de e-mail do seu Proxmox Backup Server ($hostname) está funcionando corretamente em $date."
...
echo ""
echo -e "${MENU}=== Teste de envio PBS concluído. ===${NORMAL}"
read -n 1 -s -r -p "Pressione uma tecla para continuar..."
