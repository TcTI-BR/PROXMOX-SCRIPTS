...
clear
echo -e "${MENU}=== PBS: Instalar Script Proxmox Toolkit NG para Executar no Login ===${NORMAL}"
...
PROFILE_SCRIPT_PATH="/etc/profile.d/proxmox-toolkit-launcher.sh" # New name, consistent with PVE
...
# Consider adding a note that if PVE also sets this, only one will effectively run or they might conflict.
# For now, keeping them separate but using the same profile script name.
...
{
    echo "#!/bin/bash"
    echo "# TcTI Proxmox Toolkit NG Launcher (PBS focus, but generic)"
    ...
    echo "        # This will run the launcher which then presents PVE/PBS choice"
    echo "        # cd \"$(dirname "$LAUNCHER_PATH")\" # cd to the launcher's directory" # Not strictly needed
    echo "        \"$LAUNCHER_PATH\"" # Execute the launcher
    echo "    fi"
    echo "fi"
} > "$PROFILE_SCRIPT_PATH"
...
echo ""
echo -e "${MENU}=== Instalação de script no login PBS concluída. ===${NORMAL}"
read -n 1 -s -r -p "Pressione uma tecla para continuar..."
