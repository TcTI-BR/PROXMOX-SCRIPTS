...
clear
echo -e "${MENU}=== PBS: Configuração de SWAP ===${NORMAL}"
...
echo -e "Valor atual do vm.swappiness: ${GREEN_TEXT}$current_swappiness${NORMAL}"
echo ""
...
echo "  - 10: Bom valor para muitos servidores."
...
echo ""
echo -e "${GREEN_TEXT}vm.swappiness configurado para $new_swappiness.${NORMAL}"
echo "A configuração foi aplicada imediatamente e persistirá após reinicializações."
echo -e "${MENU}=== Configuração de SWAP PBS concluída. ===${NORMAL}"
read -n 1 -s -r -p "Pressione uma tecla para continuar..."
