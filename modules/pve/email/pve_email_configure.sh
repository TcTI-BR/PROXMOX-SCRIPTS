...
clear
echo -e "${MENU}=== Configuração do Serviço de E-mail (Postfix) ===${NORMAL}"
...
if [ ! -f /etc/postfix/main.cf.BCK_proxmox_scripts ] && [ -f /etc/postfix/main.cf ]; then
    cp -f /etc/postfix/main.cf /etc/postfix/main.cf.BCK_proxmox_scripts
    echo "Backup de /etc/postfix/main.cf criado em /etc/postfix/main.cf.BCK_proxmox_scripts"
fi
if [ ! -f /etc/aliases.BCK_proxmox_scripts ] && [ -f /etc/aliases ]; then
    cp -f /etc/aliases /etc/aliases.BCK_proxmox_scripts
    echo "Backup de /etc/aliases criado em /etc/aliases.BCK_proxmox_scripts"
fi
...
# Add a line for the system user (e.g., www-data, or current user if applicable) if needed
# echo "$(whoami) $varsenderaddress" >> /etc/postfix/sender_canonical
...
if [ "$vartls" == "yes" ]; then
    ...
    postconf -e "smtp_tls_wrappermode = no" # Usually 'no' for port 587
    # For port 465 (SMTPS), smtp_tls_wrappermode should be 'yes'
    if [ "$varmailport" == "465" ]; then
        postconf -e "smtp_tls_wrappermode = yes"
    fi
    postconf -e "smtp_tls_CAfile = /etc/ssl/certs/ca-certificates.crt" # Standard CA certs path
    postconf -e "smtp_tls_security_level = encrypt" # Enforce encryption
else
...
echo "Limpando arquivo de senha temporário..."
# rm -f /etc/postfix/sasl_passwd # Original script removed it, but postmap creates .db. For debugging, can be kept.
# For security, it's better to remove it after postmap if not debugging.
# However, the original script did this, so let's keep the behavior for now, but be aware.
rm -f "/etc/postfix/sasl_passwd"


echo ""
echo -e "${MENU}=== Configuração de E-mail Concluída. ===${NORMAL}"
echo "Verifique os logs do Postfix (/var/log/mail.log) se houver problemas."
echo "É recomendado testar o envio de e-mail usando a opção 2 do menu de e-mail."
read -n 1 -s -r -p "Pressione uma tecla para continuar..."
